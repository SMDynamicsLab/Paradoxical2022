...
The stability condition doesn't depend on $I_{\ext}$ and it reads the same as Eq.\ \ref{eq.SH_stable_cond_v1}:
\begin{equation}
\begin{aligned}
(E_{\set}^2 & \alpha_{\IE} + I_{\set}^2 \alpha_{\II}) I_{\set}(W_{\EE\up} g_E - 1) < \\
(E_{\set}^2 & \alpha_{\EE} + I_{\set}^2 \alpha_{\EI}) (E_{\set} W_{\IE\up} g_E - \Theta_I g_E)
...
\noindent which is very similar to Eq.\ \ref{eq.TT_stable_cond_v1_summary} except that it has ($\Theta_E - I_{\ext}$) instead of just $\Theta_E$. From this it should be evident that the condition will still hold for any positive value of $I_{\ext}$ (right-hand side decreases).
...
\noindent which are very similar to Eqs.\ \ref{eq.FB_stable_cond_summary} except that there is a ($\Theta_E - I_{\ext}$) instead of just $\Theta_E$. From this it should be evident that including a positive $I_{\ext}$ will increase the chances that the conditions hold, because it makes the left-hand sides decrease.
